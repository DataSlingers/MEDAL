"""
Lambda_d analysis: effect of the distillation loss weight on distillation and
reconstruction loss.

Grid-searches over::

    lambda_d ∈ { 1, 5, 10, 50, 100, 500 }

repeated 10 times (different AE random seeds) → 60 trials per dataset.
Architecture is fixed at hidden_dims=[128]*3; all other architectural and
optimisation hyperparameters are taken from eval/configs.py (INIT_CONFIG)
and held constant.
...
Output
------
{dataset_key}_lambda_d_analysis_{teacher}.csv  — one row per trial, columns:
    lambda_d, seed, distill_loss, recon_loss

Example
-------
python eval/run_lambda_d_analysis.py \\
    --dataset mnist \\
    --output_dir /share/ctn/users/bnc2119/results/lambda_d_analysis/mnist \\
    --ray_storage_path /tmp/ray_lambda_d_mnist
"""
...
LAMBDA_D_GRID = [1, 5, 10, 50, 100, 500]

FIXED_HIDDEN_DIMS = [128, 128, 128]

N_SEEDS = 10

# Fixed teacher config per algorithm (held constant — we are studying lambda_d,
...
def _lambda_d_trainable(config):
    """
    Ray Tune trainable for one (lambda_d, seed) trial.
    ...
    * MEDAL.fit() reports distill_loss / recon_loss to Ray at each
    ...
    student = MEDAL(
        ...
        hidden_dims          = config.get("hidden_dims", FIXED_HIDDEN_DIMS),
...
def run_lambda_d_analysis(
    ...
    # ── teacher embedding (one, shared across all trials) ───────────────────
    ...
    # ── arch/opt params from INIT_CONFIG — everything fixed except lambda_d
    arch_cfg = {
        k: v for k, v in INIT_CONFIG[dataset_key].items()
        if k not in ("test_size", "lambda_d")
    }
    ...
    base_cfg = {
        **arch_cfg,
        ...
        "lambda_d":          tune.grid_search(LAMBDA_D_GRID),
    ...
    print(
        f"\n[{dataset_key}] Launching {len(LAMBDA_D_GRID)} lambda_d values "
        f"× {N_SEEDS} seeds = {len(LAMBDA_D_GRID) * N_SEEDS} trials "
        f"(1 GPU + 4 CPU each)\n"
    )

    analysis = tune.run(
        _lambda_d_trainable,
        name=f"lambda_d_analysis_{dataset_key}",
    ...
    col_map = {}
    for raw in ("config/lambda_d", "config/seed"):
        if raw in df.columns:
            col_map[raw] = raw.split("/")[1]
    df.rename(columns=col_map, inplace=True)

    # Fallback: unpack from the "config" dict column if present
    if "lambda_d" not in df.columns and "config" in df.columns:
        df["lambda_d"] = df["config"].apply(lambda c: c.get("lambda_d"))
        df["seed"]     = df["config"].apply(lambda c: c.get("seed"))

    keep = [c for c in ("lambda_d", "seed", "distill_loss", "recon_loss")
            if c in df.columns]
    df = df[keep].copy()

    csv_path = Path(f"{dataset_key}_lambda_d_analysis_{teacher}.csv")
    df.to_csv(csv_path, index=False)
    print(f"\nDone. Results saved to: {csv_path.resolve()}")
    print(df.groupby("lambda_d")[["distill_loss", "recon_loss"]].mean().to_string())
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "MEDAL lambda_d analysis: sweep lambda_d over "
            f"{N_SEEDS} seeds per config ({len(LAMBDA_D_GRID) * N_SEEDS} trials total)."
        ),
    ...
    parser.add_argument(
        ...
        help="Teacher algorithm (config is fixed; we are studying lambda_d).",
    )
    parser.add_argument(
