...
X_scdeed = pd.read_csv(Path(PATH_PREFIX) / f"MEDAL/comparisons/data/astro_train_pc5.csv", index_col=0).to_numpy()
...
# #     if perp == 38: continue
#     teacher_embed = np.load(Path(PATH_PREFIX) / f"drd_data/embeddings2/astro_tsne_{perp}_0_train.npy")
#     tw_df = evaluate_embedding(
#         X_high = X,
#         X_low = teacher_embed,
...
#     student = load_trained_ae(Path(PATH_PREFIX) / f"drd_data/tmp_results/chkpt_teacher_seed/astro/medal_tsne2_{perp}_tc0_0_ckpts/final.pt",
#                 input_dim=19,
...
# by_k_tsne.to_csv('astro_by_k_tsne_medal.csv')
...
#     teacher_embed = np.load(Path(PATH_PREFIX) / f"drd_data/embeddings/astro_umap_{n}_0.1_0_train_embedr.npy").squeeze(0)
#     tw_df = evaluate_embedding(
#         X_high = X,
#         X_low = teacher_embed,
#         k_values = range(5, 51, 3)
#     )
#     tw_df.update({"method": f'EMBEDR_{n}', "split": "Train"})
#     by_k_umap = pd.concat([by_k_umap, pd.DataFrame(tw_df)])
    
    
#     student = load_trained_ae(Path(PATH_PREFIX) / f"drd_data/tmp_results/compare_embedr/astro/medal_umap2_{n}_0.1_tc0_0_ckpts/final.pt",
#                 input_dim=19,
#                 hidden_dims=[512, 512, 512, 512],
#                 latent_dim=2, activation=torch.nn.SELU, batchnorm=False)
...
#             k_values = range(5, 51, 3)
#         )
#         tw_df.update({"method": f'EMBEDR_{n}', "split": "Test"})
#         by_k_umap = pd.concat([by_k_umap, pd.DataFrame(tw_df)])
            
# by_k_umap.reset_index(names='k', inplace=True)
# by_k_umap.to_csv('astro_by_k_umap_embedr.csv')