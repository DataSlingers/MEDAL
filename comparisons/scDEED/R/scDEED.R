# ============================================================
# scDEED.R (patched for SeuratObject >= 5)
# - Replaces deprecated `slot` with `layer` in Get/SetAssayData
# - Keeps the external API compatible by still accepting `slot`,
#   but maps it to `layer` internally.
# ============================================================
...
Permuted <- function(pbmc,
                     ...
                     layer = NULL,
                     slot = "scale.data",      # kept for backward-compat; mapped to `layer` if layer is NULL
                     run_pca = TRUE,
                     K) {
  
  # Back-compat: if caller still passes slot=..., use it unless layer was explicitly given
...
Distances.tSNE <- function(pbmc, pbmc.permuted, K, perplexity_score = 40,
                           pre_embedding = "pca", check_duplicates = TRUE, rerun = TRUE) {
...
Distances.UMAP <- function(pbmc, pbmc.permuted, K, pre_embedding = "pca",
                           n = 30, m = 0.3, rerun = TRUE) {
...
optimize <- function(input_data, input_data.permuted, pre_embedding, reduction.method, K,
                     ...
                     trustworthy_cutoff, check_duplicates = TRUE, rerun = TRUE) {
  ...
  int <- ifelse(length(ClassifiedCells$intermediate_cells) != 0,
                paste(ClassifiedCells$intermediate_cells, sep = ",", collapse = ","),
                "none")
  trust <- ifelse(length(ClassifiedCells$trustworthy_cells) != 0,
                  paste(ClassifiedCells$trustworthy_cells, sep = ",", collapse = ","),
                  "none")
...
scDEED <- function(input_data, K,
  ...
  if (pre_embedding != "pca" & is.na(permuted)) {
    error_message <- paste0(
      c("Stop! scDEED does not know how to calculate ",
        pre_embedding,
        ". Please calculate this pre-embedding on the permuted data and provide the object in the permuted argument. Refer to tutorial, https://github.com/JSB-UCLA/scDEED?tab=readme-ov-file#changing-input-space")
    )
    stop(error_message)
  }
  ...
  results.PCA <- suppressMessages(
    Distances.pre_embedding(input_data, input_data.permuted, K = K, pre_embedding = pre_embedding)
  )
  ...
  if (tolower(reduction.method) == "tsne") {
    ...
    if (is.null(input_data@reductions$tsne)) {
      input_data <- Seurat::RunTSNE(input_data, do.fast = TRUE, check_duplicates = check_duplicates)
    }
    ...
    if (length(perplexity) > 1) {
      ...
      original <- suppressMessages(
        foreach::`%do%`(
          foreach::foreach(p = perplexity[1], .combine = "cbind"),
          optimize(input_data, input_data.permuted, pre_embedding, reduction.method, K,
