"""
Depth analysis: effect of per-layer neuron count on distillation and
reconstruction loss.
...
Example
-------
python eval/run_depth_analysis.py \\
    --dataset mnist \\
    --output_dir /share/ctn/users/bnc2119/results/depth_analysis/mnist \\
    --ray_storage_path /tmp/ray_depth_mnist
"""
...
HIDDEN_DIMS_GRID = {
    "mnist":       {L: [w] * L for L, w in zip(DEPTHS, [305, 256, 227, 191, 169])},
    "gene_cancer": {L: [w] * L for L, w in zip(DEPTHS, [260, 256, 253, 247, 242])},
    "darmanis":    {L: [w] * L for L, w in zip(DEPTHS, [317, 256, 222, 183, 160])},
}

N_SEEDS = 10

# Fixed teacher config per algorithm (held constant — we are studying depth,
...
def run_depth_analysis(
    ...
    # ── Ray Tune config ──────────────────────────────────────────────────────
    dataset_grid = list(HIDDEN_DIMS_GRID[dataset_key].values())  # list of [w]*L lists

    base_cfg = {
        **arch_cfg,
        "hidden_dims":      tune.grid_search(dataset_grid),
    ...
    print(
        f"{len(dataset_grid)} hidden_dims configs × {N_SEEDS} seeds"
    ...
    analysis = tune.run(
        _depth_trainable,
        name=f"depth_analysis_{dataset_key}",
    ...
    df["arch_width"] = df["hidden_dims"].apply(
        lambda h: h[0] if isinstance(h, (list, tuple)) else int(str(h).split("x")[0])
    )
    df["arch_depth"] = df["hidden_dims"].apply(
        lambda h: len(h) if isinstance(h, (list, tuple)) else int(str(h).split("x")[1])
    )
    df["hidden_dims"] = df["hidden_dims"].apply(
        lambda h: f"{h[0]}x{len(h)}" if isinstance(h, (list, tuple)) else str(h)
    )

    csv_path = Path(f"{dataset_key}_depth_analysis_{teacher}.csv")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "MEDAL depth analysis: sweep hidden_dims over "
    ...
    parser.add_argument(
        ...
        help="Teacher algorithm (config is fixed; we are studying architecture depth).",
    )
    parser.add_argument(
